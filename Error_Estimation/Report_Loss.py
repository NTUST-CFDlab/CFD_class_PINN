...
def Plot_Final_LossCurve3(Case_Info, MF="Reports/"):
    ...
    LF_Scr,_ = Case_Info.Load_Loss_Function()
    LF_Types, LF_PG_Conv, LF_Setting = LF_Conversion_Layer(LF_Scr)
...
def LF_Conversion_Layer(Scr_LF):
    Total_Scr       = len(Scr_LF)
    LF_Types   = ["" for i in range(Total_Scr)] 
    LF_Weigths = [1. for i in range(Total_Scr)]
    LF_PG_Conv = [0  for i in range(Total_Scr)] 
    LF_Setting = [[] for i in range(Total_Scr)]
        
    for i in range(len(Scr_LF)):
        LF_Types[i]   = Scr_LF[i][0]
        LF_Weigths[i] = Scr_LF[i][1]
        LF_PG_Conv[i] = [i]
        LF_Setting[i] = Scr_LF[i][3]
        
        if Scr_LF[i][0] == "NR_GE":
            LF_Types[i]   = "NR"
    return LF_Types, LF_PG_Conv, LF_Setting
