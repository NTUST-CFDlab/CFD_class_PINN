...
def Generate_Points(Case_Info, CD):
    ...
    Scr_BP, Scr_DP, Scr_CP = Case_Info.Load_Point_Gen_Info()
    Scr_UP = [[] for i in range(0)]
    #_, _, _, Point_Grouping, CD.LF_Setting = Case_Info.Load_Weights_List()
    LF_Scr, _ = Case_Info.Load_Loss_Function()
    Point_Grouping = LF_Point_Grouping(LF_Scr)
    ...
    M_Plot_Points(CD, Case_Info, Point_Grouping, WF)
...
# Plot
def M_Plot_Points(CD, Case_Info, Point_Grouping, WF):
...
def LF_Point_Grouping(Scr_LF):
    PG_List = []
    for i in range(len(Scr_LF)):
        PG_List.append(Scr_LF[i][2])
    return PG_List
