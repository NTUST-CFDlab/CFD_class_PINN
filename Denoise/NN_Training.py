...
class PINN_Solver_Class():
    def Initialize_Training_Info(self, Case_Info, CD, Report_Result, DTYPE):
        ...
        self.Solver_Iter_Limit = [10000 , 300000]
        self.Adam_Lr           = tf.keras.optimizers.schedules.PiecewiseConstantDecay([1000, 3000], [1e-2, 1e-3, 1e-4])
        
        #self.LF_Types, self.LF_Weigths, self.LF_PG_Conv, PG_List, LF_Setting = Case_Info.Load_Weights_List()
        LF_Scr, self.B1_List = Case_Info.Load_Loss_Function()
        self.LF_Conversion_Layer(LF_Scr)
        ...
        #NR_Loss_ID, self.B1_List = Case_Info.Load_NR_Settings()
        #self.NR_GE_Loss_ID       = NR_Loss_ID[0]
        #self.NR_Data_Loss_ID     = [NR_Loss_ID[1]]
        self.Current_Beta        = self.B1_List[0]
    ...
    def LF_Conversion_Layer(self, Scr_LF):
        Total_Scr       = len(Scr_LF)
        self.LF_Types   = ["" for i in range(Total_Scr)] 
        self.LF_Weigths = [1. for i in range(Total_Scr)]
        self.LF_PG_Conv = [0  for i in range(Total_Scr)] 
        self.LF_Setting = [[] for i in range(Total_Scr)]
        
        self.NR_GE_Loss_ID   = 0
        self.NR_Data_Loss_ID = []
        
        for i in range(len(Scr_LF)):
            self.LF_Types[i]   = Scr_LF[i][0]
            self.LF_Weigths[i] = Scr_LF[i][1]
            self.LF_PG_Conv[i] = [i]
            self.LF_Setting[i] = Scr_LF[i][3]
        
            if Scr_LF[i][0] == "NR_GE":
                self.NR_GE_Loss_ID = [i]
                self.LF_Types[i]   = "NR"
            elif Scr_LF[i][0] == "NR_DA":
                self.NR_Data_Loss_ID.append(i)
        ...
        for Solver_Index in range(len(self.Solver_Order)):
            ...
            if self.Solver_Order[Solver_Index]=="BFGS":
                ...
                for B_Index in range(1, len(self.B1_List)):
                    self.Current_Beta = self.B1_List[B_Index]
                    self.solve_with_ScipyOptimizer(method='L-BFGS-B',
                                                   options={'maxiter': Temp_Iter,
                                                        'maxfun': 50000,
                                                        'maxcor': 50,      # previously 50
    ...
    def loss_fn(self):
        ...
        for i in range(len(self.LF_Types)):
            if self.LF_Types[i] == "GE":
                for k in range(len(self.LF_PG_Conv[i])):
                    ...
                    for j in range(1,len(self.LF_Setting[C_ID])):
                        ...
                        Weighted_Square = tf.reduce_mean(tf.square(All_Var_Pred[Var_Index]))# * self.Special_Weights[i][k][j-1])
            ...
            elif self.LF_Types[i] == "Avg_p":
                for k in range(len(self.LF_PG_Conv[i])):
                    C_ID = self.LF_PG_Conv[i][k]
                    [All_Var_Pred] = self.GE.Get_Sim_Param(self.X_C[C_ID], [self.LF_Setting[C_ID][0]])
                    for j in range(1,len(self.LF_Setting[C_ID])):
                        Var_Index = self.LF_Setting[C_ID][j]
                        #print(i, self.LF_Setting[C_ID][j], C_ID, Var_Index, self.U_C[C_ID][Var_Index])
                        Delta_BC_D = tf.square(tf.reduce_mean(All_Var_Pred[Var_Index]))
                        Loss_Per_Type.append(Delta_BC_D * self.LF_Weigths[i])# * self.Special_Weights[i][k][j-1])
                ...
                for k in range(len(self.LF_PG_Conv[ii])):
                    ...
                    for j in range(1,len(self.LF_Setting[C_ID])):
                        Var_Index = self.LF_Setting[C_ID][j]
                        Sum += tf.reduce_mean(tf.square(R_Raw[Var_Index])) #* self.Special_Weights[ii][k][j-1])
                GE_Loss = Sum * self.LF_Weigths[ii]

                # Data
                for jj in range(len(self.NR_Data_Loss_ID)):
                    Data_Loss = 0
                    ii = self.NR_Data_Loss_ID[jj]
                    for k in range(len(self.LF_PG_Conv[ii])):
                        C_ID = self.LF_PG_Conv[ii][k]
                        [All_Var_Pred] = self.GE.Get_Sim_Param(self.X_C[C_ID], [self.LF_Setting[C_ID][0]])
                        for j in range(1, len(self.LF_Setting[C_ID])):
                    ...
                    # print(Data_Loss)
                    Alpha1 = 4.2
                    DO_Constant = 2. * tf.math.log(self.Current_Beta) / tf.math.log(10.) + 0.5
                    W_DA = tf.sigmoid(4.2 * (tf.math.log(GE_Loss) / tf.math.log(10.) + 3.))
